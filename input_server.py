...
class PrivateNetworkAccessMiddleware:
    """WSGI middleware that adds the Access-Control-Allow-Private-Network
    header to ALL responses, including those generated by Flask-SocketIO.

    Flask-SocketIO handles /socket.io/ routes with its own CORS handler,
    bypassing Flask's @after_request hook.  This middleware operates at the
    WSGI level so it catches everything.

    Chrome's Private Network Access (PNA) requires this header when a
    cross-origin HTTPS iframe makes requests to localhost.
    """
    def __init__(self, wsgi_app):
        self.wsgi_app = wsgi_app

    def __call__(self, environ, start_response):
        def add_pna_header(status, headers, exc_info=None):
            headers.append(('Access-Control-Allow-Private-Network', 'true'))
            return start_response(status, headers, exc_info)
        return self.wsgi_app(environ, add_pna_header)
